...
`jin session new --worktree` でセッションを作成した際、worktree 生成直後・Claude Code 起動前にセットアップ用スクリプトを自動実行できます。依存関係のインストール、`.env` のコピーなどを済ませてから Claude を起動できるので、中途半端な状態を Claude に触らせずに済みます。
