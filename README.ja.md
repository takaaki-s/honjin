...
`config.yaml` の `keybindings.*` を編集すると、TUI を再起動せずに反映され
ます。ファイル監視には `fsnotify` を使用し、保存の約 250ms 後にデバウンス
付きでリロードして TUI 内のバインディングテーブルを再構築します。編集中
の一時的な YAML 構文エラーは無視されるので保存中のセッションが壊れる心配
はなく、`fsnotify` をサポートしない filesystem では起動時に警告ログを出し
て従来通り再起動要求の挙動にフォールバックします。

次のバインドは依然 **`jin ui` の再起動が必要** です。outer tmux
（`jin-mgr`）の起動時に配線されるため、実行中に差し替えると古い
`bind-key` が残ってしまうためです:

- `keybindings.toggle_pane`
- `keybindings.action_panel`
- `keybindings.search`（セッションフィルタポップアップ）
- `keybindings.detach`
